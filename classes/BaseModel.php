...
class BaseModel implements ArrayAccess
{
    ...
    public static function orderByAsc($id) {
        $db = new DB();
        return $db->query('SELECT * FROM ' . static::$table . ' ORDER BY '.$id. ' ASC')->fetchAll() ;
    }
    ...
    public static function deleteItens($cond,$id) {
        $db = new DB();
        $db->delete(static::$table,$cond, $id);
    }
